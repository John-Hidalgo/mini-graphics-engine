...
void RayTraceRendererGI::loadShaders() {
	...
	cubeRefractionLocs.clear();
	sphereRefractionLocs.clear();
	triangleRefractionLocs.clear();
	...
	for (int i = 0; i < 10; i++) {
		...
		cubeRefractionLocs.push_back(glGetUniformLocation(shaderProgram, ("cubeRefractions[" + std::to_string(i) + "]").c_str()));
		sphereRefractionLocs.push_back(glGetUniformLocation(shaderProgram, ("sphereRefractions[" + std::to_string(i) + "]").c_str()));
		triangleRefractionLocs.push_back(glGetUniformLocation(shaderProgram, ("triangleRefractions[" + std::to_string(i) + "]").c_str()));
...
void RayTraceRendererGI::draw() {
	...
	for (int i = 0; i < cubeCenters.size(); i++) {
		if (cubeCenterLocs[i] != -1) {
			...
			if (cubeRefractionLocs[i] != -1) {
				glUniform1f(cubeRefractionLocs[i], cubeRefractions[i]);
			}
		}
	}
	for (int i = 0; i < sphereCenters.size(); i++) {
		if (sphereCenterLocs[i] != -1) {
			...
			if (sphereRefractionLocs[i] != -1) {
				glUniform1f(sphereRefractionLocs[i], sphereRefractions[i]);
			}
		}
	}
	for (int i = 0; i < triangleV0.size(); i++) {
		if (triangleV0Locs[i] != -1) {
			...
			if (triangleRefractionLocs[i] != -1) {
				glUniform1f(triangleRefractionLocs[i], triangleRefractions[i]);
			}
		}
	}
